...
async function testHarness(options: {
  mode?: "socket" | "http";
  server: {
    head?: Buffer;
  ...
  const server = await listen(
    ...
    (socket, head) => {
      ...
      socket.on("data", (data) => options.server.data?.(data, socket));
  ...
  const serverProxy = await listen(serverProxyPort, (client) => {
    if (options.mode === "http") {
      ...
      socket.on("error", fail).on("connect", () =>
        zstdProxy({
          compress: { socket, head: options.server.head },
          to: client,
        })
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    socket
      ...
      .on("data", (data) => options.client.data?.(data, socket));
  });
...
async function listen(
  ...
  handle: (socket: Socket, head?: Buffer) => void,
  {
    mode,
    pauseOnConnect = true,
  }: { mode?: "socket" | "http"; pauseOnConnect?: boolean } = {}
) {
  if (mode === "http") {
